...
async function handleDeleteSelectedShipments(date) {
    ...
    try {
        ...
        if (result.success) {
            ...
            // تحديث الحالة المحلية بناءً على ما تم حذفه في الخلفية
            state.shipments = state.shipments.filter(s => !shipmentIdsToDelete.includes(s.id));
            const defectIdsToDelete = new Set(deletedData.defects.map(d => d.id));
            state.defects = state.defects.filter(d => !defectIdsToDelete.has(d.id));

            // تحديث مخزون المنتجات
            const allItems = deletedData.shipments.flatMap(s => s.items);
            const allDefects = deletedData.defects;

            // أولاً، أعد التوالف للمخزون
            ...
            // ثانياً، اخصم الكمية الإجمالية للفواتير
            ...
            // إظهار إشعار مع خيار التراجع
            utils.showUndoNotification('Invoices deleted.', async () => {
                utils.showLoader();
                // منطق التراجع: إعادة البيانات المحذوفة إلى الحالة وحفظها
                state.shipments.unshift(...deletedData.shipments);
                state.defects.unshift(...deletedData.defects);

                // إعادة تطبيق تغييرات المخزون (عكس الحذف)
                allDefects.forEach(defect => {
                    const product = state.products.find(p => p.id === defect.productId);
                    if (product?.colors?.[defect.color]?.sizes?.[defect.size]) {
                        product.colors[defect.color].sizes[defect.size].quantity -= defect.quantity;
                ...
                await api.saveData();
                ui.renderSuppliersPage();
                utils.showNotification('Deletion undone.', 'success');
                utils.hideLoader();
            });
        } else {
            utils.showNotification(`Error: ${result.error}`, 'error');
